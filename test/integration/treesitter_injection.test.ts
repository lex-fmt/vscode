...
integrationTest('injection highlighter detects verbatim language zones', async () => {
  ...
  // The fixture has 4 annotated verbatim blocks: python, javascript, json, rust
  // (the 5th block has no language annotation)
  assert.ok(zones.length >= 4, `Expected at least 4 injection zones, got ${zones.length}`);
