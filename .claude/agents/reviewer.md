...
- The git hooks run the full lint suite (the same command as CI) at commit and push, so do not run linters as a separate verification step. Run `pixi run lint --fix` only when you expect formatting damage, then commit and let the hook be the check. Always go through the pixi task, never a bare `shipit lint` or a raw linter: only the task runs the fleet-pinned toolchain the hook and CI enforce, and a fixer run outside it reformats correct files into a state the hook then rejects.
